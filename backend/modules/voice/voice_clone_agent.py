...
def _default_voice_storage_dir() -> str:
    """Canonical voice-clone storage: backend/data/voice_models/.

    Resolved relative to *this* file (modules/voice/) so the path is
    stable regardless of the server's CWD. Overridable via constructor
    argument or the ANT_VOICE_STORAGE_DIR env var.
    """
    env_override = os.environ.get("ANT_VOICE_STORAGE_DIR")
    if env_override:
        return env_override
    # backend/modules/voice/voice_clone_agent.py -> backend/data/voice_models/
    backend_root = Path(__file__).resolve().parent.parent.parent
    return str(backend_root / "data" / "voice_models")
...
class VoiceCloneManager:
    ...
    def __init__(self, storage_dir: str = None):
        # Use absolute path derived from this file's location so voice-clone
        # data lives in the same place regardless of server CWD. Pass an
        # explicit `storage_dir` to override (e.g. in tests).
        self.storage_dir = storage_dir or _default_voice_storage_dir()
