...
class TestRealtimeSuggestionEngine:
    ...
    def test_is_question_with_question_mark(self):
        ...
        assert self.engine.is_question("What is React?") is True
        assert self.engine.is_question("How does it work?") is True

    def test_is_question_with_question_words(self):
        """Test detection of questions starting with question words"""
        assert self.engine.is_question("Tell me about your experience") is True
        assert self.engine.is_question("Explain how you handled a conflict") is True
        assert self.engine.is_question("Describe your biggest achievement") is True

    def test_is_question_with_technical_patterns(self):
        """Test detection of technical interview patterns"""
        assert self.engine.is_question("Design a URL shortener") is True
        assert self.engine.is_question("Implement a hash map") is True
        assert self.engine.is_question("Optimize this query") is True

    def test_is_not_question_statement(self):
        """Test that statements are not detected as questions"""
        assert self.engine.is_question("I worked on React for 3 years") is False
        assert self.engine.is_question("The system uses microservices") is False
    ...
    def test_calculate_confidence_short_query(self):
        ...
        assert confidence < 0.5  # Penalized for short query
    ...
    def test_clear_buffer(self):
        ...
        assert len(self.engine.segment_buffer) == 1

        self.engine.clear_buffer()
        assert len(self.engine.segment_buffer) == 0
        assert self.engine.last_suggestion_time is None

    def test_set_min_confidence(self):
        """Test confidence threshold adjustment"""
        self.engine.set_min_confidence(0.8)
        assert self.engine.min_confidence == 0.8

        # Test clamping
        self.engine.set_min_confidence(1.5)
        assert self.engine.min_confidence == 1.0

        self.engine.set_min_confidence(-0.5)
        assert self.engine.min_confidence == 0.0
    ...
    def test_format_suggestion(self):
        ...
        assert "Tell me about yourself" in formatted
        assert "Google" in formatted
        assert "software engineer" in formatted
        assert "topics" in formatted.lower() or "Topics" in formatted
...
class TestVoiceCommandProcessor:
    ...
    def test_search_command_pattern(self):
        ...
        assert self.processor.process_command("what did i say about React") is not None
        assert self.processor.process_command("remind me about system design") is not None
        assert self.processor.process_command("search for my Python answers") is not None
        assert self.processor.process_command("find React in my history") is not None
...
class TestTranscriptSegment:
    ...
    def test_segment_creation(self):
        ...
        assert segment.text == "Test question?"
        assert segment.speaker == "interviewer"
