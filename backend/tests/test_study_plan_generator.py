...
class TestSpacedRepetitionScheduler:
    ...
    def test_calculate_next_review_subsequent(self):
        ...
        assert interval2 >= interval1  # nosec B101
    ...
    def test_poor_performance_resets_interval(self):
        ...
        assert interval == 1  # Reset to day 1  # nosec B101
    ...
    def test_interval_capped_at_year(self):
        ...
        assert interval <= 365  # nosec B101
    ...
    def test_generate_review_schedule(self):
        ...
        assert len(schedule) >= 1  # nosec B101

        # All dates should be within range
        for date, task in schedule:
            assert date >= start_date  # nosec B101
            assert date <= start_date + timedelta(days=30)  # nosec B101
...
class TestResourceLibrary:
    ...
    def test_get_resources_algorithms(self):
        ...
        assert len(resources) <= 2  # nosec B101
        if resources:
            assert all("type" in r for r in resources)  # nosec B101
    ...
    def test_get_resources_languages(self):
        ...
        assert len(resources) <= 3  # nosec B101
        if resources:
            assert all("type" in r for r in resources)  # nosec B101
    ...
    def test_get_resources_returns_all_if_less_than_count(self):
        ...
        assert len(resources) <= 10  # nosec B101
...
class TestStudyPlanGenerator:
    ...
    def test_generate_plan_basic(self):
        ...
        assert plan.duration_days == 7  # nosec B101
        assert isinstance(plan.created_at, datetime)  # nosec B101
        assert len(plan.weak_areas) >= 0  # nosec B101
    ...
    def test_generate_plan_with_graph_data(self):
        ...
        assert "React" in weak_names or "System Design" in weak_names  # nosec B101
    ...
    def test_generate_tasks_from_weak_areas(self):
        ...
        assert len(tasks) > 0  # nosec B101
        assert all(t.category == "algorithms" for t in tasks)  # nosec B101
        assert all(t.difficulty in ["easy", "medium", "hard"] for t in tasks)  # nosec B101

    def test_categorize_skill_algorithms(self):
        """Test skill categorization - algorithms"""
        assert self.generator._categorize_skill("Binary Trees") == "algorithms"  # nosec B101
        assert self.generator._categorize_skill("Dynamic Programming") == "algorithms"  # nosec B101

    def test_categorize_skill_system_design(self):
        """Test skill categorization - system design"""
        assert self.generator._categorize_skill("System Design") == "system_design"  # nosec B101
        assert self.generator._categorize_skill("Distributed Systems") == "system_design"  # nosec B101

    def test_categorize_skill_languages(self):
        """Test skill categorization - languages"""
        assert self.generator._categorize_skill("Python") == "python"  # nosec B101
        assert self.generator._categorize_skill("JavaScript") == "javascript"  # nosec B101
        assert self.generator._categorize_skill("Go") == "go"  # nosec B101

    def test_categorize_skill_default(self):
        """Test skill categorization - default"""
        assert self.generator._categorize_skill("Something Unknown") == "behavioral"  # nosec B101

    def test_estimate_time_by_difficulty(self):
        """Test time estimation by difficulty"""
        assert self.generator._estimate_time("easy") == 30  # nosec B101
        assert self.generator._estimate_time("medium") == 45  # nosec B101
        assert self.generator._estimate_time("hard") == 60  # nosec B101
        assert self.generator._estimate_time("unknown") == 45  # Default  # nosec B101

    def test_generate_theme(self):
        """Test theme generation"""
        theme = self.generator._generate_theme(["algorithms", "algorithms", "system_design"])
        assert isinstance(theme, str)  # nosec B101
        assert len(theme) > 0  # nosec B101

    def test_generate_theme_empty(self):
        """Test theme generation with empty categories"""
        theme = self.generator._generate_theme([])
        assert theme == "Mixed Review"  # nosec B101
    ...
    def test_generate_milestones(self):
        ...
        assert len(milestones) > 0  # nosec B101
    ...
    def test_adapt_plan_complete_task(self):
        ...
        if plan.sessions and plan.sessions[0].tasks:
            ...
            assert adapted_plan.completed_tasks == original_completed + 1  # nosec B101
            assert adapted_plan.progress_percentage >= 0  # nosec B101
    ...
    def test_adapt_plan_excellent_performance(self):
        ...
        if plan.sessions and plan.sessions[0].tasks:
            ...
            assert adapted_plan is not None  # nosec B101
    ...
    def test_adapt_plan_poor_performance(self):
        ...
        if len(plan.sessions) >= 2 and plan.sessions[0].tasks:
            ...
            assert new_task_count >= original_task_count  # nosec B101
...
class TestExportFormats:
    ...
    def test_export_json(self):
        ...
        assert isinstance(exported, str)  # nosec B101
        # Should be valid JSON
        import json
        data = json.loads(exported)
        assert data["user_id"] == "test_user"  # nosec B101
    ...
    def test_export_ical(self):
        ...
        assert isinstance(exported, str)  # nosec B101
        assert "BEGIN:VCALENDAR" in exported  # nosec B101
        assert "END:VCALENDAR" in exported  # nosec B101
        assert "VERSION:2.0" in exported  # nosec B101
    ...
    def test_export_markdown(self):
        ...
        assert isinstance(exported, str)  # nosec B101
        assert "# Study Plan" in exported  # nosec B101
        assert self.plan.user_id in exported  # nosec B101
...
class TestConvenienceFunctions:
    ...
    def test_adapt_plan(self):
        ...
        if plan.sessions and plan.sessions[0].tasks:
            ...
            assert adapted is not None  # nosec B101
...
class TestStudyTaskDataclass:
    ...
    def test_task_creation(self):
        ...
        assert task.id == "task1"  # nosec B101
        assert task.completed is False  # nosec B101
...
class TestStudySessionDataclass:
    ...
    def test_session_creation(self):
        ...
        assert session.total_minutes == 30  # nosec B101
        assert session.theme == "Test Theme"  # nosec B101
